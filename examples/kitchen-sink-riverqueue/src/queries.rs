...
async fn pg_advisory_xact_lock(key: i64) -> Result<bool, Box<dyn std::error::Error>> {
    let rec = sqlx::query!("
SELECT pg_advisory_xact_lock($1)
");
    Err(Box::new(std::io::Error::new(std::io::ErrorKind::Other, "not implemented")))
}
async fn pg_notify_many(
    topic: String,
    payload: Vec<String>,
...
) -> Result<ClientCreateOrSetUpdatedAtRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<ClientQueueCreateOrSetUpdatedAtManyRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<JobCancelRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
WITH locked_job AS (
    SELECT
...
async fn job_delete(id: i64) -> Result<JobDeleteRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<Vec<JobGetAvailableRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
WITH locked_jobs AS (
    SELECT
...
) -> Result<JobGetByKindAndUniquePropertiesRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM river_job
...
async fn job_get_by_kind_many(
    kind: Vec<String>,
) -> Result<Vec<JobGetByKindManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM river_job
...
async fn job_get_by_id(id: i64) -> Result<JobGetByIdRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM river_job
...
async fn job_get_by_id_many(
    id: Vec<i64>,
) -> Result<Vec<JobGetByIdManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM river_job
...
async fn job_get_stuck(
    ...
    max: i32,
) -> Result<Vec<JobGetStuckRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT id, args, attempt, attempted_at, attempted_by, created_at, errors, finalized_at, kind, max_attempts, metadata, priority, queue, state, scheduled_at, tags, unique_key, unique_states
FROM river_job
...
) -> Result<Vec<JobInsertFastManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<JobInsertFastManyNoReturningRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_job(
    args,
...
) -> Result<JobInsertFullRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_job(
    args,
...
) -> Result<JobRescueManyRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
UPDATE river_job
SET
...
async fn job_retry(id: i64) -> Result<JobRetryRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn job_schedule(
    ...
    max: i64,
) -> Result<Vec<JobScheduleRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn job_set_complete_if_running_many(
    id: Vec<i64>,
    finalized_at: Vec<chrono::DateTime<chrono::Utc>>,
) -> Result<Vec<JobSetCompleteIfRunningManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<JobSetStateIfRunningRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
WITH job_to_update AS (
    SELECT
...
) -> Result<Vec<JobSetStateIfRunningManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
WITH job_input AS (
    SELECT
...
) -> Result<JobUpdateRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
UPDATE river_job
SET
...
) -> Result<JobInsertFastManyCopyFromRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_job(
    args,
...
async fn leader_attempt_elect(
    leader_id: String,
    ttl: chrono::Duration,
) -> Result<LeaderAttemptElectRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn leader_attempt_reelect(
    leader_id: String,
    ttl: chrono::Duration,
) -> Result<LeaderAttemptReelectRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_leader(leader_id, elected_at, expires_at)
    VALUES ($1, now(), now() + $2::interval)
...
async fn leader_delete_expired() -> Result<
    LeaderDeleteExpiredRow,
...
async fn leader_get_elected_leader() -> Result<
    LeaderGetElectedLeaderRow,
    Box<dyn std::error::Error>,
> {
    let rec = sqlx::query!(
        "
...
) -> Result<LeaderInsertRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<LeaderResignRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn river_migration_delete_assuming_main_many(
    version: Vec<i64>,
) -> Result<Vec<RiverMigrationDeleteAssumingMainManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn river_migration_delete_by_line_and_version_many(
    ...
    Vec<RiverMigrationDeleteByLineAndVersionManyRow>,
    Box<dyn std::error::Error>,
> {
    let rec = sqlx::query!(
        "
...
async fn river_migration_get_by_line(
    line: String,
) -> Result<Vec<RiverMigrationGetByLineRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn river_migration_insert(
    line: String,
    version: i64,
) -> Result<RiverMigrationInsertRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn river_migration_insert_many(
    line: String,
    version: Vec<i64>,
) -> Result<Vec<RiverMigrationInsertManyRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_migration (
    line,
...
async fn river_migration_insert_many_assuming_main(
    version: Vec<i64>,
) -> Result<Vec<RiverMigrationInsertManyAssumingMainRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
INSERT INTO river_migration (
    version
...
async fn table_exists(table_name: String) -> Result<bool, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
) -> Result<QueueCreateOrSetUpdatedAtRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn queue_delete_expired(
    ...
    max: i64,
) -> Result<Vec<QueueDeleteExpiredRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn queue_get(name: String) -> Result<QueueGetRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn queue_list(
    limit_count: i32,
) -> Result<Vec<QueueListRow>, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
SELECT name, created_at, metadata, paused_at, updated_at
FROM river_queue
...
async fn queue_pause(name: String) -> Result<QueuePauseRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
...
async fn queue_resume(
    name: String,
) -> Result<QueueResumeRow, Box<dyn std::error::Error>> {
    let rec = sqlx::query!(
        "
WITH queue_to_update AS (
    SELECT name
